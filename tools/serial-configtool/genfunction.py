...
def Write_uart_handle_init(f, iface:dict):
    f.write(f"\t/* {iface['name']} Init */\n")
    f.write(f"\tStatus = CFE_SRL_HandleInit(&Handles[CFE_SRL_{iface['name']}_HANDLE_INDEXER], \"{iface['name']}\", \"{iface['DevName']}\", SRL_DEVTYPE_{iface['type'].upper()}, CFE_SRL_{iface['name']}_HANDLE_INDEXER, {iface['baudrate']});\n")
    return
...
def Write_socat_handle_init(f, iface:dict):
    f.write(f"\t/* {iface['name']} Init */\n")
    f.write(f"\tStatus = CFE_SRL_HandleInit(&Handles[CFE_SRL_{iface['name'].upper()}_HANDLE_INDEXER], \"{iface['name']}\", \"{iface['DevName']}\", SRL_DEVTYPE_UART, CFE_SRL_{iface['name'].upper()}_HANDLE_INDEXER, 115200);\n")
